-- Migration 006: add fetched_at to search_talent_complete_fn return
-- Exposes LinkedIn data freshness to the scoring pipeline (E1 recency signal).
--
...
RETURNS TABLE (
  ...
  fetched_at          TIMESTAMPTZ   -- when LinkedIn data was last fetched (data freshness signal)
) AS $$
  SELECT
    ti.github_username,
    ti.github_data,
